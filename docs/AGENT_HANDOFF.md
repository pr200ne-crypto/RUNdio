...
**最終更新**: 2026-04-09 — **新エージェント向けのセッション引き継ぎ**は [`HANDOFF_NEXT_AGENT_2026-04-09.md`](./HANDOFF_NEXT_AGENT_2026-04-09.md) を最初に読む。**正規の GitHub・本番 URL・Supabase** は [`CANONICAL.md`](./CANONICAL.md)（GitHub: [pr200ne-crypto/RUNdio](https://github.com/pr200ne-crypto/RUNdio)、本番: [ru-ndio.vercel.app](https://ru-ndio.vercel.app)）。**次タスク一覧**: [`NEXT_TASKS.md`](./NEXT_TASKS.md)。
...
| ファイル | 役割 |
|----------|------|
| `system_requirements.md` | システム要件定義（**版 0.2**・Web デモ向け） |
| `detailed_requirements_specification.md` | 詳細要件定義（テンプレ準拠、**版 0.2**、Mermaid・API 例あり） |
| `continuation-checkpoint-2026-04-04.md` | **エージェント続き用**（NotebookLM 待ち・並行タスク・再開プロンプト） |
...
- `notebooklm_demo_audio_scenario_prompts.md` … **NotebookLM** 取り込み用マスター仕様（§2 ソース／§3 カスタム指示）
...
> `@docs/AGENT_HANDOFF.md` と `@docs/continuation-checkpoint-2026-04-04.md` を読み、このプロジェクトを再開してください。

その後 **`docs/output/detailed_requirements_specification.md`（版 0.2）** と **`docs/input/mvp-scope.md`** を読み、**課題デモ（ブラウザ・低コスト）**に合わせて実装を進めること。

**デモ音声・NotebookLM・ElevenLabs** を扱う場合は、追加で [`docs/AGENT_HANDOFF_AUDIO_ELEVENLABS.md`](./AGENT_HANDOFF_AUDIO_ELEVENLABS.md) を読むこと。
