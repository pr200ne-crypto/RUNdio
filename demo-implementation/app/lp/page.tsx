...
import { Play, ArrowRight, Mail, Lock, MapPin, Headphones, TrendingUp, LogOut, Home } from 'lucide-react'
      ...
      <nav className="sticky top-4 mx-4 bg-white/70 backdrop-blur-xl z-50 rounded-full border border-white/40 shadow-[0_8px_30px_rgb(0,0,0,0.04)] px-4 py-3 flex items-center justify-between">
        ...
        {user ? (
          <button
            ...
            className="px-5 py-2 bg-blue-600 text-white rounded-full text-sm font-bold hover:bg-blue-700 transition-colors shadow-md"
          >
            ホームへ戻る
          </button>
        ) : (
        ...
        <div className="max-w-md mx-auto bg-white p-6 rounded-[3rem] shadow-2xl shadow-blue-900/5 border border-slate-100">
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-black text-slate-900 tracking-tight">
              {user ? 'ログイン中' : (isLogin ? 'おかえりなさい' : 'RUNdioをはじめる')}
            </h2>
            <p className="mt-2 text-slate-500 text-sm">
              {user ? 'アプリをお楽しみください' : (isLogin ? 'アカウントにログインして再開' : '無料でアカウントを作成')}
            </p>
          </div>
          ...
          {user ? (
            <div className="space-y-4">
              <button
                onClick={() => router.push('/home')}
                className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition-colors shadow-md flex items-center justify-center gap-2"
              >
                <Home size={20} />
                ホームへ戻る
              </button>
              <button
                ...
                className="w-full py-4 bg-white text-red-600 border-2 border-red-100 rounded-2xl font-bold hover:bg-red-50 transition-colors flex items-center justify-center gap-2"
              >
                <LogOut size={20} />
                ログアウト
              </button>
